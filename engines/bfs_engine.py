...
class BFS8PuzzleEngine(BaseAlgorithmEngine):
    key = "bfs"
    display_name = "Breadth-First Search (BFS)"
    source_filename = "bfs_algorithm.py"
    source_title = "BFS Algorithm Source"
    TRACE_PROGRAM = [
        "if finished: return",
        "if queue empty: fail",
        "current = queue.popleft()",
        "if current == goal: success",
        "for neighbor in neighbors(current): enqueue unseen",
    ]
    ...
    def step(self):
        ...
        if not self.search_state["queue"]:
            ...
            trace_step.append(self._push_trace(2, "Queue is empty -> no solution found."))
        ...
        current = self.search_state["queue"].popleft()
        ...
        for neighbor in get_neighbors(current):
            if neighbor not in self.search_state["seen"]:
                ...
                self.search_state["queue"].append(neighbor)
                pushed += 1
